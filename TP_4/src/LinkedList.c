...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	if (this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
	{
		...
		if (nuevoNodo != NULL)
		{
			...
			this->size++; // actualizo el tam del linkedlist
			returnAux = 0;
		}
	}
    return returnAux;
...
{
    return addNode(this, ll_len(this), pElement ); // no es necesario validar si el addNode ya lo valida!!
}
...
void* ll_get(LinkedList* this, int index)
{
	...
	auxNode = getNode(this, index); // no es necesario validad, el getNode ya lo valida
	if (auxNode != NULL)
	{
...
//        // OPCION 2
//        // voy yendo desde el final hacia adelante
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
	...
	if (this != NULL && pFunc != NULL && order >= 0 && order <= 1){
		for (int i =0; i < tam-1; i++)
		{
			...
			{
				if (( pFunc( ll_get(this, i), ll_get(this, j) ) > 0 && order ) || ( pFunc( ll_get(this, i), ll_get(this, j) ) < 0 && !order ) ) // aca va la funcion para comparar que tiene que devolver 1 o 0, o 1 o -1 para swapear
				{
					aux = ll_get(this, i);
